...
class ParsingNet(nn.Cell):
    ...
    def construct(self, x):
        ...
        if self.use_aux and self.training:
            ...
            x3 = P.interpolate(x3, size=(x3.shape[2] * 2, x3.shape[3] * 2), mode='bilinear', align_corners=True)   # MS2.x: ResizeBilinear 已移除
            x4 = self.aux_header4(fea)
            x4 = P.interpolate(x4, size=(x4.shape[2] * 4, x4.shape[3] * 4), mode='bilinear', align_corners=True)   # MS2.x: ResizeBilinear 已移除
