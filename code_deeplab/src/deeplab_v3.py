...
class ASPPPooling(nn.Cell):
    ...
    def construct(self, x):
        size = self.shape(x)
        # MS2.x：原写 nn.AvgPool2d(size[2])，kernel 是张量，会回落 AICPU 的 TF 算子并报
        # 'Aicpu kernel execute failed, retCode=0x2a'。等价写法是全局自适应池化 → 1x1。
...
class DeepLabV3(nn.Cell):
    ...
    def construct(self, x):
        ...
        out = self.resnet(x)
        out = self.aspp(out)
        # MS2.x：ops.ResizeBilinear 已移除，改用 ops.interpolate（显式传 size）
        # 注意：size 要用 x.shape（纯 int）。若用 self.shape(x) 的返回值，TBE 编译
        # ResizeBilinearV2 会失败（Failed to compile Op ResizeBilinearV2）。
        out = ops.interpolate(out, size=(x.shape[2], x.shape[3]), mode='bilinear', align_corners=True)
        return out
